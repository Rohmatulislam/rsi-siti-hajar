...
export default function RadiologyVisitsPage() {
  const [radiologyVisits, setRadiologyVisits] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  const [visitType, setVisitType] = useState<'all' | 'outpatient' | 'inpatient'>('all');

  // Ambil data kunjungan radiologi dari API
  useEffect(() => {
    const fetchRadiologyVisits = async () => {
      try {
        setLoading(true);
        
        // Panggil API untuk mendapatkan data kunjungan radiologi
        const response = await fetch('/api/khanza/radiology-visits');
        if (!response.ok) {
          throw new Error('Failed to fetch radiology visit data');
        }
        
        const data = await response.json();
        setRadiologyVisits(data);
      } catch (error) {
    ...
    fetchRadiologyVisits();
  }, []);

  const filteredRadiologyVisits = radiologyVisits.filter(visit => {
    const matchesSearch = (visit.patient_name && visit.patient_name.toLowerCase().includes(searchTerm.toLowerCase())) ||
      (visit.examination_name && visit.examination_name.toLowerCase().includes(searchTerm.toLowerCase())) ||
  ...
  // Kalkulasi statistik
  const outpatientRadVisits = radiologyVisits.filter(v => v.visit_type === 'rawat_jalan').length;
  const inpatientRadVisits = radiologyVisits.filter(v => v.visit_type === 'rawat_inap').length;
  const totalExaminations = radiologyVisits.length;
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="text-2xl">Manajemen Kunjungan Radiologi</CardTitle>
        ...
        <CardContent>
              ...
              <CardContent className="p-4">
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold">{totalExaminations}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="p-4">
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold">{outpatientRadVisits}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="p-4">
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold">{inpatientRadVisits}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="relative w-96">
              ...
              <Input
                placeholder="Cari kunjungan radiologi (nama pasien, pemeriksaan)..."
                value={searchTerm}
              ...
              {loading ? (
                <TableRow>
                  <TableCell colSpan={8} className="text-center">
                    Memuat data kunjungan radiologi...
                  </TableCell>
                </TableRow>
              ) : filteredRadiologyVisits.length > 0 ? (
                filteredRadiologyVisits.map((visit, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>{visit.examination_name}</TableCell>
                    <TableCell>
                      {visit.result_status ? (
                ...
                <TableRow>
                  <TableCell colSpan={8} className="text-center">
                    Tidak ada data kunjungan radiologi ditemukan
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
