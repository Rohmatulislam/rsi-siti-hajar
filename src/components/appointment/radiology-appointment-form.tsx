...
const radiologyAppointmentSchema = z.object({
...
type RadiologyAppointmentFormValues = z.infer<typeof radiologyAppointmentSchema>;

interface RadiologyAppointmentFormProps {
  onComplete?: () => void;
}

export function RadiologyAppointmentForm({ onComplete }: RadiologyAppointmentFormProps) {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [showSuccess, setShowSuccess] = useState(false);
  const [appointmentData, setAppointmentData] = useState<any>(null);

  const form = useForm<RadiologyAppointmentFormValues>({
    resolver: zodResolver(radiologyAppointmentSchema),
    defaultValues: {
      name: '',
      phone: '',
      email: '',
  ...
  const handleSubmit = async (data: RadiologyAppointmentFormValues) => {
    setIsSubmitting(true);
    try {
      // Simulasi pengiriman data ke backend
      console.log("Data pendaftaran radiologi:", data);
      ...
      setAppointmentData({
        ...
        location: "Lantai 2, RSI Siti Hajar Mataram",
        contact: "(0370) 623xxx"
      });
  ...
  return (
    <Card className="border-0 shadow-lg rounded-2xl bg-card">
      <CardHeader>
        <CardTitle className="text-2xl font-bold text-emerald-700">Formulir Pendaftaran Radiologi</CardTitle>
      </CardHeader>
      <CardContent>
        <Form {...form}>
